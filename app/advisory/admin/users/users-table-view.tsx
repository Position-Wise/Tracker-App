...
function UserEditDrawer({
  ...
  const [selectedRole, setSelectedRole] = useState(roleFormValue)
  const effectiveRole = selectedRole === "__current__" ? normalizedRole : selectedRole
  const canUseSystemPlans = isAdminRole(effectiveRole)
  const availablePlans = useMemo(
    () =>
      plans.filter((plan) => {
        const planType = (plan.plan_type ?? "").trim().toLowerCase()
        if (canUseSystemPlans) return true
        return planType !== "system"
      }),
    [canUseSystemPlans, plans]
  )
  const selectedPlanExists = availablePlans.some((plan) => plan.id === selectedPlan)
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              className="h-9 w-full rounded-md border border-input bg-background px-2 text-sm"
            >
              {selectedPlan === "__current__" || !selectedPlanExists ? (
                <option value="__current__">
                  {currentPlanName
