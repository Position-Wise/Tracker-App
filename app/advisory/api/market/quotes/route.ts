...
export async function GET(request: Request) {
  ...
  const rateLimit = checkRateLimit(`market-quotes:${access.user.id}`, {
    windowMs: 60_000,
    maxRequests: 45,
  })
  ...
  if (cached && Date.now() - cached.cachedAt <= QUOTE_CACHE_TTL_MS) {
    return NextResponse.json(cached.payload, {
      headers: {
        "Cache-Control": "private, max-age=0, s-maxage=8, stale-while-revalidate=20",
      },
    })
  }
  ...
  try {
    ...
    if (supabaseResult && supabaseResult.quotes.length > 0) {
      quoteResponseCache.set(cacheKey, { cachedAt: Date.now(), payload: supabaseResult })
    ...
    if (v7Quotes && v7Quotes.length >= symbols.length) {
      ...
      quoteResponseCache.set(cacheKey, { cachedAt: Date.now(), payload })
