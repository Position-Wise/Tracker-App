...
export default function DashboardTabView({
  ...
  const [liveBroadcasts, setLiveBroadcasts] = useState(broadcasts)
  ...
  const refreshFeed = useCallback(async () => {
    try {
      ...
      if (Array.isArray(payload.broadcasts)) {
        setLiveBroadcasts(payload.broadcasts)
      }
      if (Array.isArray(payload.suggestions)) {
        setLiveSuggestions(payload.suggestions)
      }
    } catch (error) {
      console.error("Dashboard feed refresh failed:", error)
    }
  }, [])
  ...
  const initialFeedbackByBroadcast = useMemo(() => {
    return liveBroadcasts.reduce<Record<string, "profit" | "loss" | null>>((acc, broadcast) => {
  ...
  const [selectedTab, setSelectedTab] = useState<DashboardTab>("announcement")
  const activeTab = useMemo<DashboardTab>(() => {
    if (availableTabs.some((tab) => tab.key === selectedTab)) {
      return selectedTab
    }
    for (const tab of DASHBOARD_TABS) {
      if (availableTabs.some((t) => t.key === tab.key)) {
        return tab.key
      }
    }
    return "announcement"
  ...
  const tabFilteredBroadcasts = useMemo(() => {
    ...
    return liveBroadcasts.filter((broadcast) => broadcast.dashboard_tab === activeTab)
  }, [activeTab, availableTabs.length, liveBroadcasts])
