...
import CustomPlanInquiryForm from "@advisory/components/inquiries/custom-plan-inquiry-form";
import SubscribeForm from "@advisory/components/subscribe/subscribe-form";
...
type FlowVariant = "full" | "payment_only";
...
export default function SubscriptionOnboardingFlow({
  ...
  const isPaymentOnlyFlow = flowVariant === "payment_only";
  ...
  const steps = isPaymentOnlyFlow
    ? ([PAYMENT_STEP, PROOF_STEP, RELAX_STEP] as const)
    : isEditMode
      ? isCustomIntent
    ...
    <section className="max-w-4xl mx-auto space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {isPaymentOnlyFlow
            ? "Use the QR below and upload your screenshot to finish this quickly."
            : isBlocked
        ...
        <CardContent className="space-y-5">
          ...
          {proofStepIndex >= 0 && effectiveStepIndex === proofStepIndex ? (
            <div className="space-y-4 rounded-lg border border-border/70 bg-muted/20 p-5">
              <p className="text-sm text-muted-foreground">
                ...
                {isPaymentOnlyFlow
                  ? ". Once submitted, admin can review it directly."
                  : "."}
              </p>
          ...
          {hasLocalSuccessStep && effectiveStepIndex === lastStepIndex ? (
            <div className="space-y-4 rounded-lg border border-emerald-300/50 bg-emerald-50 p-5 text-emerald-900">
              ...
              <p className="text-sm">
                {isPaymentOnlyFlow
                  ? "Great work. Admin will review your proof and update your access shortly."
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button asChild size="sm">
                  <Link href="/wait-approval">Check approval status</Link>
                </Button>
              </div>
            </div>
          ) : null}
