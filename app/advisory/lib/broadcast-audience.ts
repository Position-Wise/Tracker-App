...
export type BroadcastAudienceType = "announcement" | "trader" | "investor" | "users"
...
export function resolveBroadcastAudienceTypeForEditor(params: {
  ...
  const normalizedBroadcast = normalizeBroadcastType(params.broadcastType)
  if (normalizedBroadcast === "announcement") {
    if (normalizedAudience === "all") return "announcement"
    if (normalizedAudience === "admin") return "users"
  }
...
export function requiresBroadcastAudienceTypeConversion(params: {
  ...
  const resolved = resolveBroadcastAudienceTypeForEditor({
